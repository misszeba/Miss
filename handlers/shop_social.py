...
def register_social_handlers(bot):
    ...
    def process_broadcast(message, bot):
        ...
        if not buyers:
            bot.send_message(message.chat.id, "❌ No buyers found to send broadcast.")
            return
        ...
        bot.send_message(message.chat.id, f"✅ Successfully Sent to {count} buyers.")
    ...
    def start_rating(call):
        ...
        try:
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="✍️ <b>Rate Product:</b>", reply_markup=kb, parse_mode="HTML")
        except:
            bot.send_message(call.message.chat.id, "✍️ <b>Rate Product:</b>", reply_markup=kb, parse_mode="HTML")
    ...
    def set_star_rating(call):
        ...
        msg = bot.send_message(call.message.chat.id, f"⭐ <b>{rating} Stars Selected!</b>\nNow send your review text:", parse_mode="HTML")
    ...
    def process_review_text(message, bot):
        ...
        bot.send_message(message.chat.id, "✅ Review Posted Successfully!")
        
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("🔙 Back to Product", callback_data=f"sh_view_{data['shop_id']}_{data['prod_id']}"))
        bot.send_message(message.chat.id, "Thank you for your feedback!", reply_markup=kb)
    ...
    def view_reviews(call):
        ...
        text = f"⭐ <b>Recent Reviews</b>\n\n"
        for r in reviews[-5:]: 
            text += f"👤 <b>{r['name']}</b> ({r['rating']}⭐)\n💬 <i>{r['text']}</i>\n\n"
...
def post_product_to_channel(bot, channel_id, prod, shop_name, shop_owner_id, bot_username):
    try:
        ...
        if not media_list: 
            ...
            m = media_list[0]
            if m["type"] == "photo": 
                bot.send_photo(channel_id, m["file_id"], caption=caption, parse_mode="HTML")
            else: 
                bot.send_video(channel_id, m["file_id"], caption=caption, parse_mode="HTML")
        else:
            album = []
            for i, m in enumerate(media_list):
                cap = caption if i == 0 else ""
                if m["type"] == "photo": 
                    album.append(InputMediaPhoto(m["file_id"], caption=cap, parse_mode="HTML"))
                elif m["type"] == "video": 
                    album.append(InputMediaVideo(m["file_id"], caption=cap, parse_mode="HTML"))
