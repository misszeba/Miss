...
def api_request(method, user_id, path="", data=None):
    ...
    # ব্রাঞ্চ সাপোর্ট যুক্ত করা হয়েছে (ডিফল্ট: main)
    branch = creds.get('branch', 'main')
    path = path.strip("/")
    
    # GET রিকোয়েস্টের জন্য ref ব্যবহার করতে হয় ব্রাঞ্চ ডিক্লেয়ার করতে
    ...
    try:
        if method == "GET":
            ...
            # PUT/DELETE এর জন্য পেলোডে ব্রাঞ্চ দিতে হয়
            if data: data['branch'] = branch
...
# নতুন ব্রাঞ্চ লিস্ট মেনু
def show_branch_list(bot, chat_id, message_id=None):
    creds = load_user_data(chat_id)
    url = f"https://api.github.com/repos/{creds['user']}/{creds['repo']}/branches"
    try:
        resp = requests.get(url, headers=get_headers(creds['token']))
        branches = resp.json() if resp.status_code == 200 else []
        
        text = f"🌿 **Select Branch for `{creds['repo']}`:**\n(Current: `{creds.get('branch', 'main')}`)"
        kb = InlineKeyboardMarkup(row_width=2)
        buttons = [InlineKeyboardButton(f"🎋 {b['name']}", callback_data=f"ugh_set_branch:{b['name']}") for b in branches]
        kb.add(*buttons)
        kb.row(InlineKeyboardButton("⬅️ Back to Browser", callback_data="ugh_nav:refresh"))

        bot.edit_message_text(text, chat_id, message_id, reply_markup=kb, parse_mode="Markdown")
    except:
        bot.send_message(chat_id, "❌ Error fetching branches.")

def show_browser(bot, chat_id, message_id=None):
    state = GH_STATE.get(chat_id, {})
    current_path = state.get("path", "")
    creds = load_user_data(chat_id)
    current_branch = creds.get('branch', 'main') # ব্রাঞ্চ ডাটা গেট
    ...
    # ব্রাঞ্চ চেঞ্জ বাটন যুক্ত করা হলো
    kb.row(*controls)
    kb.row(InlineKeyboardButton("🌿 Branch", callback_data="ugh_change_branch"), InlineKeyboardButton("➕ Upload", callback_data="ugh_act:create"))
...
def register_handlers(bot):
    ...
    def set_repo(call):
        ...
        save_user_data(call.from_user.id, {"repo": repo_name, "branch": "main"}) # ডিফল্ট ব্রাঞ্চ main সেট করা হলো
        GH_STATE[call.from_user.id] = {"path": ""}
        show_browser(bot, call.message.chat.id, call.message.message_id)

    # ব্রাঞ্চ পরিবর্তনের হ্যান্ডলারসমূহ
    @bot.callback_query_handler(func=lambda c: c.data == "ugh_change_branch")
    def change_branch_callback(call):
        show_branch_list(bot, call.message.chat.id, call.message.message_id)

    @bot.callback_query_handler(func=lambda c: c.data.startswith("ugh_set_branch:"))
    def set_branch_callback(call):
        branch_name = call.data.split(":")[1]
        save_user_data(call.from_user.id, {"branch": branch_name})
        GH_STATE[call.from_user.id] = {"path": ""}
        bot.answer_callback_query(call.id, f"Branch set to: {branch_name}")
        show_browser(bot, call.message.chat.id, call.message.message_id)
    ...
    def start_upload(call):
        ...
        # "Done" বাটন যোগ করা হয়েছে যাতে ইউজার কাজ শেষ করে ব্রাউজারে ফিরতে পারে
        kb = InlineKeyboardMarkup().add(InlineKeyboardButton("✅ Done / Back", callback_data="ugh_nav:refresh"))
        bot.edit_message_text("📂 **Bulk Upload Active**\n\nআপনি এখন এক বা একাধিক ফাইল পাঠাতে পারেন। সব ফাইল পাঠানো শেষ হলে নিচের 'Done' বাটনে ক্লিক করুন।", call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
    ...
    def process_inputs(message):
        ...
        # ১. একাধিক ফাইল আপলোড লজিক (Bulk Upload)
        elif step == "up_wait" and message.document:
            fname = message.document.file_name
            fpath = f"{path}/{fname}" if path else fname
            
            # SHA চেক (যাতে একই নামের ফাইল থাকলে সেটি রিপ্লেস হয়)
            ...
            res, info = api_request("PUT", user_id, fpath, {"message": "Bulk Up", "content": encoded, "sha": sha})
            
            if res:
                bot.reply_to(message, f"✅ `{fname}` uploaded successfully.")
            else:
                bot.reply_to(message, f"❌ Failed to upload `{fname}`: {info}")
            
            # লক্ষ্য করুন: এখানে step = None করা হয়নি। ফলে ইউজার আরও ফাইল পাঠাতে পারবে।

        # ২. টেক্সট থেকে ম্যানুয়াল ফাইল তৈরি (আগের মতোই)
        elif step == "up_wait" and message.text:
            GH_STATE[user_id]["temp_name"] = message.text.strip()
            GH_STATE[user_id]["step"] = "content_wait"
            bot.reply_to(message, f"📝 Target: `{GH_STATE[user_id]['temp_name']}`\n👉 Send content (File or Text) now.")

        # কন্টেন্ট রিসিভ করা (ম্যানুয়াল মোড)
            ...
            # সেভ করার পর আবার আপলোড মোডে রাখা হলো যাতে আরও আপলোড করা যায়
            GH_STATE[user_id]["step"] = "up_wait" 
