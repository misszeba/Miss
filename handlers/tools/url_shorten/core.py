...
def register_url_handlers(bot):
    ...
    # --- ৩. মেইন লিঙ্ক হ্যান্ডলার (UPDATED FOR TWITTER FIX) ---
    # ✅ এখানে টুইটার বা এক্স এর লিংক হলে এই হ্যান্ডলার কাজ করবে না।
    # ফলে এটি স্কিপ হয়ে আপনার Twitter Downloader প্লাগিনে চলে যাবে।
    @bot.message_handler(regexp=r"^https?://", func=lambda m: m.chat.type == "private" and "twitter.com" not in m.text and "x.com" not in m.text)
    def handle_link_private(m): 
        process_url(bot, m)
